...
namespace Tron
{
	public class Tron
	{
		...
		private void InitialisePlayers(bool isAI)
		{
			player1 = new Player(PlayerDirections.Right, 0, xDim / 2);
			input.Player1KeysPressed += (player1 as Player).ChangeDirection;
			if (!isAI)
			{
				player2 = new Player(PlayerDirections.Left, yDim - 1, xDim / 2);
				input.Player2KeysPressed += (player2 as Player).ChangeDirection;
			}
			else
